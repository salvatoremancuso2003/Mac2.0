 ...
 * @author fplacanica
 */
public class C_FreeTax_NationAmount {
    ...
    final String intestazionePdf = "VAT Refund - Nation & Currency";
...
//    public static final String logo = "web/resource/logocl.png";
     ...
     * @return
     */
    public Document receipt(C_FreeTax_NationAmount_value cmfb, ArrayList<String> colonne, boolean firstTime, boolean lastTime, Document document) {
        ...
        try {
            ...
            ArrayList<C_FreeTax_NationAmount_value> dati = cmfb.getDati();
            ...
            if (ft) {
                ...
                phraset = new Phrase();
                phraset.add(new Chunk(cmfb.getNation(), f4_bold));
            ...
            for (int j = 0; j < dati.size(); j++) {
                ...
                C_FreeTax_NationAmount_value temp =  dati.get(j);
    ...
    public String main(String path, String d3, String d4,
        ...
        try {
            C_FreeTax_NationAmount nctl = new C_FreeTax_NationAmount();

            C_FreeTax_NationAmount_value pdf = new C_FreeTax_NationAmount_value();
            ...
            ArrayList<C_FreeTax_NationAmount_value> dati = dbm.list_C_FreeTax_NationAmount_value(filiali, data1, data2);
            if (dati.size() > 0) {
//
                for (int i = 0; i < nat.size(); i++) {

                    ArrayList<C_FreeTax_NationAmount_value> dati_cu = new ArrayList<>();
                    for (int j = 0; j < dati.size(); j++) {
                        if (dati.get(j).getNation().equals(nat.get(i)[0])) {
...
//
                    ...
                    pdf.setDataA(d4);
                    pdf.setDati(dati_cu);
                    pdf.setNation(nat.get(i)[0] + " " + nat.get(i)[1]);
//
//       
                    if (i == nat.size() - 1) {
                        lastTime = true;
    ...
    public int receiptexcel(C_FreeTax_NationAmount_value cmfb, ArrayList<String> colonne, boolean firstTime, boolean lastTime, HSSFSheet sheet, int cntriga, HSSFCellStyle style1, HSSFCellStyle style2, HSSFCellStyle style3, HSSFCellStyle style4, HSSFCellStyle style3left, HSSFCellStyle style4left, HSSFCellStyle style3center) {
        ...
        try {
            ...
            for (int j = 0; j < dati.size(); j++) {
                ...
                C_FreeTax_NationAmount_value temp = (C_FreeTax_NationAmount_value) dati.get(j);
            ...
            if (qtyPrevYear > 0 && qtyYear > 0) {
                f70.setCellValue(formatMysqltoDisplay(roundDoubleandFormat(qtyVal / qtyPrevYear, 2)));
            } else {
