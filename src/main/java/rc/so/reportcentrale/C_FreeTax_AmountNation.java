 ...
 * @author fplacanica
 */
public class C_FreeTax_AmountNation {
     ...
     *
     */
    public static float[] columnWidths2 = new float[]{40f, 10f, 10f, 10f, 10f, 10f, 10f, 10f, 10f, 10f};
    ...
    final String intestazionePdf = "Vat Refund - Currency & Nation";
...
//    public static final String logo = "web/resource/logocl.png";
     ...
     * @return
     */
    public Document receipt(C_FreeTax_AmountNation_value cmfb, ArrayList<String> colonne, boolean firstTime, boolean lastTime, Document document) {
        ...
        try {
            ...
            ArrayList<C_FreeTax_AmountNation_value> dati = cmfb.getDati();
            ...
            if (ft) {
                ...
                phraset = new Phrase();
                phraset.add(new Chunk(cmfb.getCurrency(), f4_bold));
            ...
            for (int j = 0; j < dati.size(); j++) {
                ...
                C_FreeTax_AmountNation_value temp = dati.get(j);
                ...
                phraset = new Phrase();
                phraset.add(new Chunk(temp.getNation(), f3_normal));
                cellt = new PdfPCell(phraset);
                cellt.setHorizontalAlignment(ALIGN_LEFT);
                cellt.setBorder(BOTTOM);
                table4.addCell(cellt);
    ...
    public String main(String path, String d3, String d4,
        ...
        try {
            C_FreeTax_AmountNation nctl = new C_FreeTax_AmountNation();

            C_FreeTax_AmountNation_value pdf = new C_FreeTax_AmountNation_value();
//

            boolean firstTime = true;
            boolean lastTime = false;

            File pdffile = new File(path + generaId(50) + "C_VatRefund_AmountNation.pdf");
            ...
            ArrayList<C_FreeTax_AmountNation_value> dati = dbm.list_C_FreeTax_AmountNation_value(filiali, data1, data2, nat);

            if (dati.size() > 0) {

                ArrayList<Currency> cu = dbm.list_figures(true);

                for (int i = 0; i < cu.size(); i++) {

                    ArrayList<C_FreeTax_AmountNation_value> dati_cu = new ArrayList<>();
                    for (int j = 0; j < dati.size(); j++) {
                        if (dati.get(j).getCurrency().equals(cu.get(i).getCode())) {
                            dati_cu.add(dati.get(j));
                        }
                    }
                    ...
                    pdf.setDataA(d4);
                    pdf.setDati(dati_cu);
                    pdf.setCurrency(cu.get(i).getCode() + " " + cu.get(i).getDescrizione());
    ...
    public int receiptexcel(C_FreeTax_AmountNation_value cmfb, ArrayList<String> colonne, boolean firstTime, boolean lastTime, HSSFSheet sheet, int cntriga, HSSFCellStyle style1, HSSFCellStyle style2, HSSFCellStyle style3, HSSFCellStyle style4, HSSFCellStyle style3left, HSSFCellStyle style4left, HSSFCellStyle style3center) {
        ...
        try {
            ...
            if (ft) {
                ...
                f1bis.setCellValue(cmfb.getCurrency());
            ...
            for (int j = 0; j < dati.size(); j++) {
                ...
                C_FreeTax_AmountNation_value temp = (C_FreeTax_AmountNation_value) dati.get(j);

                //   listaTot.add(temp);
                Cell f1bis = row6.createCell(1);
                f1bis.setCellStyle(style4left);
                f1bis.setCellValue(temp.getNation());
            ...
            f32.setCellValue("Total Currency");
            ...
            if (lastTime) {
                ...
                f72.setCellValue("Total Local Currency");
                ...
                f442.setCellValue("Total Currency");
    ...
    public String mainexcel(String path, String d3, String d4,
            ...
            ArrayList<String> alcolonne, ArrayList<String> filiali, ArrayList<String[]> nat) {
        try {
            File pdffile = new File(path + generaId(50) + "C_VAT_Refund_Currency_Nation.xls");
            HSSFWorkbook workbook = new HSSFWorkbook();
            HSSFSheet sheet = workbook.createSheet("C_VAT_Refund_Currency_Nation");
...
//
            ...
            if (dati.size() > 0) {
                ...
                ArrayList<Currency> cu = dbm.list_figures();
                int nriga = 1;
                for (int i = 0; i < cu.size(); i++) {
