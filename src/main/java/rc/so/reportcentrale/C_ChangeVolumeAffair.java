 ...
 * @author fplacanica
 */
public class C_ChangeVolumeAffair {
     ...
     *
     */
    public static float[] columnWidths2 = new float[]{30f, 10f, 10f, 10f, 10f, 10f};

    /**
     *
     */
    public static float[] columnWidths2False = new float[]{30f, 10f, 10f, 10f, 10f};
    ...
    final String intestazionePdf = "Change Turnover";
...
//    public static final String logo = "web/resource/logocl.png";
     ...
     * @throws IOException
     */
    public Document receipt(C_ChangeVolumeAffair_value cmfb, ArrayList<String> colonne,ArrayList<String> colonneriepilogo, boolean firstTime, boolean lastTime, Document document, boolean checkTrans, boolean checkRiepilogo) throws DocumentException, BadElementException, IOException {
        ...
        try {
            ...
            if (true) {
                ...
                ArrayList<C_ChangeVolumeAffair_value> dati = cmfb.getDati();
                Phrase phraset;
                PdfPCell cellt;
                PdfPTable table3;
                ...
                ArrayList<C_ChangeVolumeAffair_value> lista = cmfb.getDati();

                for (int x = 0; x < lista.size(); x++) {

                    C_ChangeVolumeAffair_value temp = lista.get(x);
            ...
            if (checkRiepilogo) {
                ...
                if (true) {
                    ...
                    PdfPCell[] listFinale = new PdfPCell[colonneriepilogo.size()];
                    //mi scandisco le colonne
                    for (int c = 0; c < colonneriepilogo.size(); c++) {
                        Phrase phraset1 = new Phrase();
                        phraset1.add(new Chunk(colonneriepilogo.get(c), f4_bold));
                        PdfPCell cellt1 = new PdfPCell(phraset1);
                        cellt1.setHorizontalAlignment(ALIGN_RIGHT);
                        cellt1.setBorder(BOTTOM | TOP);
                        cellt1.setFixedHeight(15f);
    ...
    public String main(String path, String d3, String d4, String data1, String data2, ArrayList<String> alcolonne,
            ArrayList<String> colonneriepilogo, ArrayList<String> filiali,
            boolean checkTrans, boolean checkRiepilogo, ArrayList<Branch> allenabledbr) {
        try {
            C_ChangeVolumeAffair nctl = new C_ChangeVolumeAffair();
            C_ChangeVolumeAffair_value pdf = new C_ChangeVolumeAffair_value();
...
//        ArrayList<String> filiali = Engine.getC_Branch_ChangeVolumeAffair();
            ...
            File pdffile = new File(path + generaId(50) + "C_ChangeVolumeAffair.pdf");
...
//            }
            ...
            for (int i = 0; i < count; i++) {
                ...
                ArrayList<C_ChangeVolumeAffair_value> dati = dbm.list_C_ChangeVolumeAffair_value(data1, data2, filiali.get(i), allenabledbr);
                pdf.setDataDa(d3);
                pdf.setDataA(d4);
                pdf.setDati(dati);
                if (i == count - 1) {
     ...
     * @return
     */
    public int receiptexcel(C_ChangeVolumeAffair_value cmfb, ArrayList<String> colonne,ArrayList<String> colonneriepilogo, boolean firstTime, boolean lastTime, boolean checkTrans, boolean checkRiepilogo, HSSFSheet sheet, int cntriga, HSSFCellStyle style1, HSSFCellStyle style2, HSSFCellStyle style3, HSSFCellStyle style4,HSSFCellStyle style4bis,HSSFCellStyle style3left, HSSFCellStyle style4left) {
...
//                document.newPage();
            ...
            if (true) {
                ...
                for (int x = 0; x < lista.size(); x++) {
                    ...
                    f3.setCellValue(formatMysqltoDisplay(temp.getBuy()));

                    Cell f4 = row6.createCell(4);
                    f4.setCellStyle(style4);
                    f4.setCellValue(formatMysqltoDisplay(temp.getSell()));
    ...
    public String mainexcel(String path, String d3, String d4, String data1, String data2, ArrayList<String> alcolonne,ArrayList<String> colonneriepilogo, ArrayList<String> filiali,
            boolean checkTrans, boolean checkRiepilogo, ArrayList<Branch> allenabledbr) {
        try {
            File pdffile = new File(path + generaId(50) + "ChangeTurnover.xls");
            HSSFWorkbook workbook = new HSSFWorkbook();
            HSSFSheet sheet = workbook.createSheet("ChangeTurnover");
            ...
            style4left.setBorderBottom(THIN);
             ...
             HSSFCellStyle style4bis = (HSSFCellStyle) workbook.createCellStyle();
            style4bis.setAlignment(LEFT);
            style4bis.setBorderTop(THIN);
            style4bis.setBorderBottom(THIN);
                   style4bis.setFont(font4);
...
//            }
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (!dati.isEmpty()) {
                    nriga = nctl.receiptexcel(pdf, alcolonne,colonneriepilogo, firstTime, lastTime, checkTrans, checkRiepilogo, sheet, nriga, style1, style2, style3, style4,style4bis,style3left, style4left);
                    one = true;
                     firstTime = false;
                }
