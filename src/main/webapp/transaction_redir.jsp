...
<html lang="en">
    ...
    <head>
        <meta charset="utf-8" />
        <title>Mac2.0 - Visual Document Tr.</title>
                ...
                <div class="page-content">
                    ...
                    <%
                        ...
                        if (it != null) {
                            ...
                            Client cl = Engine.query_Client_transaction(it.getCod(), it.getCl_cod(), null);
                            ArrayList<Document> alldoc = Engine.list_all_ypedoc_tra();
                            ArrayList<Ch_transaction_doc> doc = Engine.get_list_tr_doc_view(cod);
                    %>
                    <div class="row">
                        <div class="col-md-11">
                            <h3 class="page-title">Transaction <%=ty%> <small><b>List documents</b></small></h3>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                                        ...
                                        <%
                                            for (int i = 0; i < doc.size(); i++) {
                                                ...
                                                String view = "class='btn btn-default popovers' "
                                                        + "href='Download?type=view_doc_tr_reprint&tr="
                                                        + dc.getCodtr() + "&cod=" + dc.getCodice_documento() + "'";
                                        %>
                                        <li class="mt-list-item done">
                                            ...
                                            <div class="list-datetime"> 
                                                <div class="btn-group btn-group-circle">
                                                    <a <%=Utility.sanitize(view)%> target="_blank"
                            ...
                            <hr>
                            <%if (Utility.validatemail(cl.getEmail())) {%>
                            <form method="POST" action="Operazioni">
                                <input type="hidden" name="type" value="sendmailclient" />
                                <input type="hidden" name="redir" value="redir" />
                                <input type="hidden" name="codtr" value="<%=Utility.sanitize(cod)%>" />
                                <input type="hidden" name="maildest" value="<%=Utility.sanitize(cl.getEmail())%>" />
                                <button type="submit" class="btn btn-danger popovers" container='body' data-trigger='hover' 
                                        data-container='body' data-placement='top' data-content="Client Email: <%=Utility.sanitize(cl.getEmail())%>">
                                    <i class="fa fa-envelope-o"></i> SEND EMAIL WITH DOCUMENTS</button>
                            </form>
                            <%}%>
                            <%if (Utility.getValue_request(request, "esito", false, "").equals("OK")) {%>
                            <hr>
                            <div class="alert alert-success">
                                <strong>Success!</strong> The operation was performed successfully.
                            </div>
                            <%} else if (Utility.getValue_request(request, "esito", false, "").equals("KO")) {%>
                            <hr>
